<script lang="ts">
  ...
  async function handleSubmit() {
    ...
    responseText =
      "AI が頑張って考えています…(それなりに時間がかかるので気長に待ってください。)";
  ...
  <form class="w-full">
    ...
    <button
      ...
      class="text-white bg-{count <= 0
        ...
        ? 'gray-400'
        : 'blue-800'} focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 mt-6 text-center"
      disabled={checking || count <= 0}
    >
      {#if checking === true}
        <svg
      ...
      送信</button
    >
    <span class="text-gray-500 ml-2">残り{count}回</span>
  </form>
